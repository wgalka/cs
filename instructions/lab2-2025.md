...
Typy proste przechowujące wartości całkowite w C#
...
Typ stałoprzecinkowy - `decimal` przechowuje liczby jako liczby całkowite z przesuniętą kropką dziesiętną (stała skala), dzięki czemu nie traci precyzji przy obliczeniach finansowych.
...
- Struktura: Jest typem wartościowym. Gdy przypisujesz strukturę do zmiennej lub przekazujesz ją do metody, tworzona jest jej kopia, a nie referencja.
    ...
    - Nie może mieć domyślnego konstruktora. Struktura ma zawsze konstruktor bez parametrów, który ustawia pola na wartości domyślne (np. 0, false, null w przypadku referencji). Można natomiast zdefiniować konstruktor z parametrami.
