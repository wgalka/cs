...
1. Pola (ang. fields) -  zmienne zadeklarowane w zakresie klasy. Mogą przechowywać dane, takie jak liczby, teksty lub obiekty innych klas. Przykład: `private int wiek;`. <mark>Niezainicjalizowanie pól klasy spowoduje że będą one przechowywać domyślne wartości.</mark>
...
7. Indeksatory	Indeksatory umożliwiają dostęp do obiektów klasy w sposób przypominający tablice. Przykład: public string this[int index] umożliwia indeksowanie obiektu klasy.

8. Konstruktory	Konstruktory są specjalnymi metodami, wywoływanymi podczas tworzenia obiektu. Służą do inicjowania danych obiektu, np. public Klasa(int a, int b).

9. Finalizatory	Finalizatory (metody ~NazwaKlasy) są wywoływane, gdy obiekt jest usuwany z pamięci, pomagają w zwolnieniu zasobów (rzadko używane w C#).
